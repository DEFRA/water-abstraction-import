...
experiment('Licence Import: Queue Licences System job', () => {
  let notifierStub
  ...
  experiment('.createMessage', () => {
    test('formats a message for PG boss', async () => {
      const message = QueueLicencesSystemJob.createMessage()

      expect(message).to.equal({
        name: 'licence-import.queue-licences-system',
        options: {
          singletonKey: 'licence-import.queue-licences-system',
          expireIn: '1 hours'
        }
      })
    })
  ...
  experiment('.handler', () => {
    experiment('when the job is successful', () => {
      test('a message is logged', async () => {
        await QueueLicencesSystemJob.handler()

        const [message] = notifierStub.omg.lastCall.args

        expect(message).to.equal('licence-import.queue-licences-system: started')
      })
      ...
      test('resolves with an array of licences to import', async () => {
        const result = await QueueLicencesSystemJob.handler()
    ...
    experiment('when the job fails', () => {
      ...
      test('logs an error message', async () => {
        await expect(QueueLicencesSystemJob.handler()).to.reject()

        expect(notifierStub.omfg.calledWith(
          'licence-import.queue-licences-system: errored', err
        )).to.equal(true)
      })

      test('rethrows the error', async () => {
        const err = await expect(QueueLicencesSystemJob.handler()).to.reject()
  ...
  experiment('.onComplete', () => {
    ...
    experiment('when the job succeeds', () => {
      ...
      test('a message is logged', async () => {
        await QueueLicencesSystemJob.onComplete(messageQueue, job)

        const [message] = notifierStub.omg.lastCall.args

        expect(message).to.equal('licence-import.queue-licences-system: finished')
      })
      ...
      experiment('but an error is thrown', () => {
        ...
        test('rethrows the error', async () => {
          const error = await expect(QueueLicencesSystemJob.onComplete(messageQueue, job)).to.reject()
