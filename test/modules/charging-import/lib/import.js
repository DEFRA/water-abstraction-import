  ...
  experiment('importChargingData', () => {
    ...
    experiment('when there are no verification errors', () => {
      ...
      test('runs each query in sequence', async () => {
        ...
        expect(pool.query.getCall(5).args[0]).to.equal(chargingQueries.createChargeVersionGuids);
        expect(pool.query.getCall(6).args[0]).to.equal(chargingQueries.createChargeElementGuids);
        expect(pool.query.getCall(7).args[0]).to.equal(chargingQueries.createChargeAgreementGuids);
        expect(pool.query.getCall(8).args[0]).to.equal(chargingQueries.importChargeVersions);
        expect(pool.query.getCall(9).args[0]).to.equal(chargingQueries.importChargeElements);
        expect(pool.query.getCall(10).args[0]).to.equal(chargingQueries.importChargeAgreements);
        expect(pool.query.getCall(11).args[0]).to.equal(chargingQueries.cleanupChargeElements);
        expect(pool.query.getCall(12).args[0]).to.equal(chargingQueries.cleanupChargeVersions);
        expect(pool.query.getCall(13).args[0]).to.equal(chargingQueries.updateChargeVersionsLicenceId);
        expect(pool.query.getCall(14).args[0]).to.equal(returnVersionQueries.importReturnVersions);
        expect(pool.query.getCall(15).args[0]).to.equal(returnVersionQueries.importReturnRequirements);
        expect(pool.query.getCall(16).args[0]).to.equal(returnVersionQueries.importReturnRequirementPurposes);
      });
    });
