...
async function go () {
  ...
  try {
    global.GlobalNotifier.omg('companies-import.import started')
    ...
    calculateAndLogTimeTaken(startTime, 'companies-import.import complete', { count })
  } catch (error) {
    global.GlobalNotifier.omfg('companies-import.import errored', error, { count })
    throw error
  }
}
...
async function _import (parties, count) {
  ...
  for (let i = 0; i < count; i += batchSize) {
    if (i === progress) {
      ...
      global.GlobalNotifier.omg(`companies-import.import progress (${i} of ${count})`)
    }
