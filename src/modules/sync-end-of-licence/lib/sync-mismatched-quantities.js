...
wrls_ended_licences AS (
  ...
  WHERE
    ...
    AND LEAST(l.expired_date, l.lapsed_date, l.revoked_date) IS NOT NULL
),
wrls_return_requirements AS (
  ...
  INNER JOIN
    wrls_ended_licences welc
    ON welc.wrls_licence_id = rv.licence_id
  WHERE
...
wrls_latest_submissions AS (
  ...
  WHERE
    r."source" = 'NALD'
    AND r."status" <> 'due'
    AND r.end_date = wrr.licence_end_date
