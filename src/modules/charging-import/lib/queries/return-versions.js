...
insert into water.return_versions
  (licence_id, version_number, start_date, end_date, status, external_id, date_created, date_updated)
    ...
    select 
      ...
      NOW() as date_created,
      NOW() as date_updated
    from import."NALD_RET_VERSIONS" nrv
    join import."NALD_ABS_LICENCES" nl on nrv."AABL_ID"=nl."ID" AND nrv."FGAC_REGION_CODE"=nl."FGAC_REGION_CODE"
    join water.licences l on l.licence_ref=nl."LIC_NO" 
...
const importReturnRequirements = `
...
join water.return_versions rv on concat_ws(':', nrf."FGAC_REGION_CODE", nrf."ARVN_AABL_ID", nrf."ARVN_VERS_NO")=rv.external_id
...
const importReturnRequirementPurposes = `
...
from import."NALD_RET_FMT_PURPOSES" nrp
...
join water.return_requirements r on r.external_id = concat_ws(':', nrp."FGAC_REGION_CODE", nrp."ARTY_ID")
