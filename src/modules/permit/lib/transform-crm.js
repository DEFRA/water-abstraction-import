...
function buildCRMPacket (licenceData, licenceRef, licenceId) {
  ...
  const metadata = _buildCRMMetadata(currentVersion)
  metadata.contacts = _contactsFormatter(findCurrent(licenceData.data.versions), licenceData.data.roles)
 ...
 */
function _buildCRMContactMetadata (currentVersion) {
 ...
 */
function _buildCRMMetadata (currentVersion) {
  if (!currentVersion) {
    return {
      IsCurrent: false
    }
  ...
  const contact = _buildCRMContactMetadata(currentVersion)
 ...
 */
function _contactsFormatter (currentVersion, roles) {
  if (!currentVersion) {
    return []
  }
 ...
 */
function _pruneNullString (data) {
  const mappedValues = {}
