...
async function go (log = false) {
  ...
  try {
    ...
    if (hasAlreadyRun) {
      global.GlobalNotifier.omg('sync-end-of-period: skipped')
    ...
    if (log) {
      calculateAndLogTimeTaken(startTime, 'sync-end-of-period: complete')
    }
  } catch (error) {
    global.GlobalNotifier.omfg('sync-end-of-period: errored', { currentRegion }, error)
