...
async function _cleanLines () {
  const query = `
    WITH invalid_summer_return_logs AS (
      SELECT
        ir.return_id
      FROM
        "returns"."returns" ir
      WHERE
        ...
        AND EXTRACT(MONTH FROM ir.end_date) = 11
        AND ir.metadata->>'isSummer' = 'true'
    )
    DELETE FROM "returns".lines l WHERE l.version_id IN (
      ...
      FROM
        "returns".versions v
      INNER JOIN invalid_summer_return_logs ON
        invalid_summer_return_logs.return_id = v.return_id
    );
  `
...
async function _cleanVersions () {
  const query = `
    ...
    DELETE FROM "returns".versions v WHERE v.return_id IN (
      SELECT
        invalid_summer_return_logs.return_id
      FROM
        invalid_summer_return_logs
    );
  `
