...
const deleteAllReturnsData = async returnId => {
  ...
  const deleteReturnRequirementPurposesQuery = 'delete from water.return_requirement_purposes where return_requirement_id::varchar in (SELECT return_requirement_id::varchar FROM water.return_requirements where return_version_id = (SELECT return_version_id from returns.versions where return_id = $1))';
